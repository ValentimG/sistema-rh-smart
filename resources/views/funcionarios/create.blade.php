...
<head>
    ...
    <title>Novo Funcionario — SMART RH</title>
...
<body>
    ...
    <main class="pg">
        <div class="section-title">Novo Funcionario</div>
        ...
        <form method="POST" action="{{ route('funcionarios.store') }}">
            ...
            <div class="form-section">
                ...
                <div class="form-group"><label class="form-label">Nome completo</label><input type="text" name="nome" class="form-input" value="{{ old('nome') }}" required></div>
                <div class="form-row">
                    <div class="form-group"><label class="form-label">Data de Nascimento</label><input type="date" name="data_nascimento" class="form-input" value="{{ old('data_nascimento') }}"></div>
                    <div class="form-group"><label class="form-label">Sexo</label><select name="sexo" class="form-select"><option value="">Selecionar...</option><option value="masculino" {{ old('sexo')=='masculino'?'selected':'' }}>Masculino</option><option value="feminino" {{ old('sexo')=='feminino'?'selected':'' }}>Feminino</option><option value="outro" {{ old('sexo')=='outro'?'selected':'' }}>Outro</option></select></div>
                </div>
                <div class="form-row">
            ...
            <div class="form-section">
                ...
                <div class="form-row">
                    <div class="form-group"><label class="form-label">E-mail</label><input type="email" name="email" class="form-input" value="{{ old('email') }}" required></div>
                    <div class="form-group"><label class="form-label">CPF</label><input type="text" name="cpf" class="form-input" value="{{ old('cpf') }}" required></div>
                </div>
                <div class="form-group"><label class="form-label">Endereco</label><input type="text" name="endereco" class="form-input" value="{{ old('endereco') }}" required></div>
            </div>

            <div class="form-section">
                <div class="form-section-title">Dados Profissionais</div>
                <div class="form-row">
                    <div class="form-group"><label class="form-label">Cargo</label><input type="text" name="cargo" class="form-input" value="{{ old('cargo') }}" required></div>
                    <div class="form-group"><label class="form-label">Tipo de Contrato</label><select name="tipo_contrato" class="form-select"><option value="clt" {{ old('tipo_contrato')=='clt'?'selected':'' }}>CLT</option><option value="pj" {{ old('tipo_contrato')=='pj'?'selected':'' }}>PJ</option><option value="estagio" {{ old('tipo_contrato')=='estagio'?'selected':'' }}>Estagio</option></select></div>
                </div>
                <div class="form-row">
                    <div class="form-group"><label class="form-label">Data de Admissao</label><input type="date" name="data_admissao" class="form-input" value="{{ old('data_admissao') }}" required></div>
                    <div class="form-group"><label class="form-label">Tipo</label><select name="tipo" class="form-select"><option value="funcionario" {{ old('tipo')=='funcionario'?'selected':'' }}>Funcionario</option><option value="gestor" {{ old('tipo')=='gestor'?'selected':'' }}>Gestor</option></select></div>
                </div>
            </div>
            ...
            <div class="form-section">
                ...
                <div class="form-row">
                    <div class="form-group"><label class="form-label">Salario Base (R$)</label><input type="number" step="0.01" name="salario_base" class="form-input" value="{{ old('salario_base') }}"></div>
                    <div class="form-group"><label class="form-label">Carga Horaria Mensal (h)</label><input type="number" name="carga_horaria_mensal" class="form-input" value="{{ old('carga_horaria_mensal', 220) }}"></div>
                </div>
            </div>
